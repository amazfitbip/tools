...
with open(fileName, mode='rb') as file: # b is important -> binary
	...
	#https://docs.python.org/3/library/struct.html#format-strings
	fileHeader = fileContent[0:6]
	version = ord(fileContent[6:7])

	if fileHeader != "HMDIAL":
		print "file isn't a resource file. Exiting"
		os.exit(1)
	print "file is a Haumi resource file"
	print "version %d" % version
	
	sizepos=ord(fileContent[41:42])
	max_rsrc = ord(fileContent[41+sizepos:42+sizepos])
	resoffset = fileContent.find('\x42\x4d\x64\x00')
	resoffset = resoffset - 4 * max_rsrc
...
#pack
if args.pack:
	header_res = map(ord,fileContent[0:resoffset])
	#extend the array to make space for number of element and resource's index
	header_res.extend([0]*( 4 * max_rsrc))
	for i in range(4):
		header_res[resoffset + i] = (max_rsrc >> 8*i) & 0xFF
	...
	for index in range(max_rsrc):
		for i in range(4):
			...
			header_res[resoffset + index  * 4+i] = (offset >> 8*i) & 0xFF
		#print "resource %3d | addr: %x " % ( index, offset )
